...
N2_all <- read.csv('Bristol_adult_rep_allblocks.csv')
...
all_rep <-ggplot(data=all_long, aes(x=factor(Day, levels = level_order), y=value, group=Treatment, color=Treatment))+
  geom_jitter(alpha = 0.2, position = position_jitterdodge(jitter.width = 0.2, dodge.width = 0.5))+
  stat_summary(fun.data="mean_cl_boot", geom="errorbar", size = 1, width=0.0, position = position_dodge(0.5)) +
  stat_summary(fun.data="mean_cl_boot", geom="point", size = 3, position = position_dodge(0.5)) +
  stat_summary(fun.data="mean_cl_boot", geom="line",  size=1, position = position_dodge(0.5)) +
...
N2_all <- N2_all %>%
  select(-c(Block, Strain))


L <- matrix(nrow = nrow(N2_all), ncol = 3)


for (i in 1:nrow(N2_all)){
  Les <- matrix(0, ncol = 12, nrow = 12)
  diag(Les[-1,]) <- rep(1, 11) # add the 1s for survival probability diagonally
  Fert <- c(0,0, as.numeric(as.vector(N2_all[i,][3:12]))) #the columns in data that has the reproductive counts
  Fert[is.na(Fert)] <- 0 #makes all NAs into 0s
  Les[1,] <- c(Fert)
  class(Les) <- "leslie.matrix"
  Lambda <- popbio::eigen.analysis(Les)$lambda1
  L[i, 1:3] <- c(paste0(N2_all$Treatment[i]), paste0(N2_all$ID[i]), Lambda)
...
Lambda_all_dab <- mean_diff(Lambda_all)

## Lambda dabestr plot
Lambda_plot_all <- dabest_plot(Lambda_all_dab, FALSE, swarm_label = 'Lambda', raw_marker_spread = 1, custom_palette = 'npg', swarm_x_text = 12, swarm_y_text = 16, contrast_y_text = 16, contrast_x_text = 12, raw_marker_alpha = 0.3, tufte_size = 1)
Lambda_plot_all


## LRS
totalrep<-na.omit(as.data.frame.table(tapply(all_long$value,list(all_long$Treatment, all_long$ID),sum)))
...
LRS_all <- dabest_plot(Totrep_dab, FALSE, swarm_label = 'LRS', raw_marker_spread = 1, custom_palette = 'npg', swarm_x_text = 12, swarm_y_text = 16, contrast_y_text = 16, contrast_x_text = 12, raw_marker_alpha = 0.3, tufte_size = 1)

LRS_all

all_repro <- ggarrange(all_rep, 
                         ggarrange(Lambda_plot_all, LRS_all, ncol = 2),
                         nrow = 2,
                         common.legend = TRUE)
all_repro

ggsave('all_repro.pdf', plot = all_repro,
       width = 10, height = 10, units = 'in')
